...
| **attest_action** | `POST /v1/attest_action` | Looks the action up in a system of record and returns a decision plus a signed Claim Credential. REST requires `procedure` and `text` **or** `document` (not both); `policy_ground` uses `grounding_references`. MCP is a separate tool schema (see below). | Verifications |
